...
if uploaded_files and actualizar:
    ...
    for i in range(len(df_export)):
        ...
        pdf.multi_cell(0, 8, f"{row['Archivo']} -> Distancia: {row['Distancia Euclidiana']:.2f} | Coseno: {row['Similitud de Coseno']:.3f} | Interpretación: {row['Interpretación']}")
    ...
    output_pdf_bytes = pdf.output(dest='S').encode('latin1', errors='ignore')
    st.download_button(
